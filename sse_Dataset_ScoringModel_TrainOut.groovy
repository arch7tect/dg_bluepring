...
def entity = session.createQuery("from sse.Dataset where name = :name").setParameter("name", "ScoringModel_TrainOut").uniqueResult() 
...
entity.datasets.add(session.createQuery("from sse.Notebook where name = :name").setParameter("name", "ScoringModel_DataTransformDev").uniqueResult())
